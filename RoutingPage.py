		...
		for index, p in enumerate(rid["parts"]):
			...
			if index == 0:
				...
				fromStationRow = ft.Row([
					ft.Row([
						ft.Text(" " + pData["fromTime"].strftime("%H:%M"), width=40),
						ft.Container(width=10, height=25, border=fromStationBorder, padding=ft.padding.all(0),
									 margin=ft.margin.only(top=20)),
						ft.Text(pData["fromStation"], size=15)
					], spacing=15),
					ft.Row([ft.Icon(ft.icons.ARROW_FORWARD_IOS, color=ft.colors.INVERSE_SURFACE, size=18)], spacing=5),
				], alignment=ft.MainAxisAlignment.SPACE_BETWEEN)
			...
			if index != len(rid["parts"]) - 1:
				...
				toStationRow = ft.Row([
					ft.Row([
						ft.Column([
							ft.Text(" " + pData["toTime"].strftime("%H:%M"), width=40),
							ft.Text(" " + nextStationTime.strftime("%H:%M"), width=40)
						], spacing=0),
						ft.Column([
